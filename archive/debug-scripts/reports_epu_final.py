"""
Reports EPU (End-to-End Product Usecase) Test Suite - Final Version
...
class ReportsEPUTestSuite:
    ...
    async def test_run_details_drill_down(self, page: Page) -> bool:
        ...
        try:
            ...
            first_row_locator = None
            for selector in row_selectors:
                rows = page.locator(selector)
                count = await rows.count()
                print(f"   Trying selector '{selector}': found {count} rows")
                if count > 0:
                    first_row_locator = rows.nth(0)  # Get the first row using nth(0)
                    print(f"   Using first row from selector '{selector}'")
                    break
            
            if first_row_locator is None:
                raise Exception("No table rows found to click")
            
            # Click the first row
            clicked = await self.safe_click(first_row_locator, "first row")
            if not clicked:
            ...
            try:
                ...
                for selector in alt_panels:
                    ...
                    if count > 0:
                        try:
                            await expect(alt_panel.nth(0)).to_be_visible(timeout=2000)
                            details_panel = alt_panel.nth(0)
                            panel_visible = True
    ...
    async def test_tab_navigation_in_details_panel(self, page: Page) -> bool:
        ...
        try:
            ...
            if not panel_visible:
                ...
                first_row_locator = page.locator('tbody tr').nth(0)
                row_count = await page.locator('tbody tr').count()
                if row_count > 0:
                    await self.safe_click(first_row_locator, "first row")
        ...
        try:
            ...
            try:
                ...
                if await close_button.count() > 0 and await close_button.is_visible():
                    ...
                    print("   [OK] Used Escape key to close any open panels")
            except:
    ...
    async def test_action_buttons_functionality(self, page: Page) -> bool:
        ...
        try:
                ...
                if clicked:
                    ...
                    for selector in panel_selectors:
                        ...
                        if await panel.count() > 0:
                            try:
                                await expect(panel.nth(0)).to_be_visible(timeout=2000)
                                details_opened = True
            ...
            try:
                ...
                if await runs_table.count() > 0:
                    await expect(runs_table.nth(0)).to_be_visible(timeout=2000)
                    table_visible = True
