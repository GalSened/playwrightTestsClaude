...
export class SmartWeSignKnowledge {
   ...
   */
  async processQuery(query: string, forcedLanguage?: string): Promise<SmartResponse> {
    ...
    try {
      // Enhanced language detection (use forced language if provided)
      const languageResult = forcedLanguage && forcedLanguage !== 'auto'
        ? { language: forcedLanguage as 'hebrew' | 'english' | 'mixed', confidence: 1.0, patterns: { hebrew: 0, english: 0, mixed: 0 } }
...
Enhanced answer:`;
  ...
  async chat(question: string, language?: string): Promise<any> {
    try {
      const response = await this.processQuery(question, language);
