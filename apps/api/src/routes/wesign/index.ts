...
router.post('/test/run', async (req, res) => {
  try {
    ...
    const testPath = `C:/Users/gals/seleniumpythontests-1/playwright_tests/${testFile}`;
    ...
    const testExecution = spawn(pythonPath, args, {
      ...
      env: {
        ...process.env,
        PYTHONPATH: 'C:/Users/gals/seleniumpythontests-1/playwright_tests'
      }
    });
...
router.get('/tests', async (req, res) => {
  try {
    ...
    const WESIGN_TEST_PATH = 'C:/Users/gals/seleniumpythontests-1/playwright_tests/tests';
    const testDirs = ['auth', 'documents', 'signing', 'contacts', 'templates',
                     'bulk_operations', 'distribution', 'reports', 'user_management'];

    const tests = [];

    for (const dir of testDirs) {
      const dirPath = path.join(WESIGN_TEST_PATH, dir);
      try {
...
router.get('/health', async (req, res) => {
  try {
    ...
    try {
      const { spawn } = require('child_process');
      const python = spawn('C:/Users/gals/AppData/Local/Programs/Python/Python312/python.exe', ['--version']);
    ...
    const fs = require('fs');
    checks.wesignTestsExists = fs.existsSync("C:/Users/gals/seleniumpythontests-1/playwright_tests");

    // Check if Playwright is available
    try {
      const { spawn } = require('child_process');
      const playwright = spawn('C:/Users/gals/AppData/Local/Programs/Python/Python312/python.exe', ['-c', 'import playwright; print("OK")']);
      ...
      await new Promise((resolve) => {
        ...
        playwright.on('close', (code: number) => {
          checks.playwrightInstalled = code === 0 && output.trim() === 'OK';
          resolve(code);
        });
