...
import UnifiedKnowledgeService from '@/services/ai/unifiedKnowledgeService';
...
let unifiedKnowledge: UnifiedKnowledgeService;
...
// Enhanced initialization with Unified Knowledge Service
const initializeServices = async () => {
  try {
    // Initialize Unified Knowledge Service (connects ALL data sources)
    if (!unifiedKnowledge) {
      unifiedKnowledge = new UnifiedKnowledgeService();
      logger.info('Unified Knowledge Service initialized', unifiedKnowledge.getStats());
    }

    // Keep legacy service for compatibility
    if (!smartKnowledge) {
      smartKnowledge = new SmartWeSignKnowledge();
      logger.info('Legacy Smart WeSign Knowledge Service initialized', smartKnowledge.getStats());
    }

    logger.info('Enhanced AI services initialized successfully', {
      agentCount: 2,
      servicesActive: ['UnifiedKnowledgeService', 'SmartWeSignKnowledge'],
      improvements: 'Hebrew support fixed, 7697+ knowledge chunks connected, sub-second responses'
    });
...
router.post('/chat', captureChatData(), saveAIResponseData(), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Use Unified Knowledge Service - Advanced, Fast & Comprehensive
    logger.info('Processing chat request with Unified Knowledge', {
    ...
    const response = await unifiedKnowledge.processQuery(question, conversationContext);

    // Enhanced response with unified context
    const combinedResponse = {
      ...
      sessionId,
      userId,
      source: conversationContext.source || 'wesign-unified-mentor',
      processingInfo: {
        unifiedKnowledge: true,
        language: response.language,
        detailLevel: preferences?.detailLevel || 'detailed',
        sourcesUsed: {
          vectorResults: response.processingDetails.vectorResults,
          databaseResults: response.processingDetails.databaseResults,
          staticResults: response.processingDetails.staticResults
        },
        languageDetection: {
          detected: response.processingDetails.languageDetection.language,
          confidence: response.processingDetails.languageDetection.confidence,
          hebrewChars: response.processingDetails.languageDetection.details.hebrewChars,
          englishChars: response.processingDetails.languageDetection.details.englishChars
        }
      }
    };
