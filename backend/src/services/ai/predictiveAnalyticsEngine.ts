...
export interface TestExecutionPrediction {
  ...
  wesignPredictions?: {
    workflowFailureRisk: Record<string, number>;
    componentStabilityScore: Record<string, number>;
    hebrewUIIssuesProbability: number;
    businessImpactScore: number;
    authenticationRisk: number;
    documentProcessingRisk: number;
    signingWorkflowRisk: number;
  };
}
...
export class PredictiveAnalyticsEngine {
  ...
  private models: {
    ...
    wesignWorkflowPrediction?: tf.Sequential;
  };
  ...
  constructor(dbPath?: string) {
    ...
    this.initializeWeSignPredictiveIntelligence();
  }

  /**
   * Initialize WeSign-specific predictive intelligence
   */
  private async initializeWeSignPredictiveIntelligence(): Promise<void> {
    try {
      logger.info('Initializing WeSign-aware predictive analytics');
      this.codeStructure = await this.codebaseAnalyzer.analyzeFullCodebase();

      logger.info('WeSign predictive analytics initialized', {
      ...
      logger.warn('Failed to initialize WeSign predictive intelligence:', error);
